...
- `run_persona_creation_workflow(instruction)` — delegate to the Persona & Identity department to create a new influencer end-to-end (character, personality, backstory, sample images), including a human review step. Use only when the user wants a brand-new influencer.
...
4. **Handle any human-review pause** from the persona workflow by relaying the draft to the user and feeding their decision back in.
