...
- **Check past research before you start digging.** Other topics may have already turned up findings relevant to yours. Use `skim_previous_findings` to see the titles/descriptions of notes left by earlier research runs, then `read_previous_finding` to pull the full notes for any topic that looks relevant. Reuse what's already been verified instead of re-researching it from scratch — but still confirm it's still current before relying on it.
- **Delegate deep, narrow digging to a subagent instead of doing it inline.** If a specific sub-question needs many searches or pages of raw content to resolve, use `spawn_subagent` rather than burning your own context on the raw search noise — it investigates independently and hands back only a finished write-up. It runs in the background: `spawn_subagent` returns a task ID immediately, not the result, so keep working on other angles and only call `check_subagent_status` once you're actually ready for that answer. Launch several in one turn to cover independent angles in parallel. A subagent sees only the instruction you give it, so make each one fully self-contained (no reference to "the topic" or earlier conversation).
...
- **No `spawn_subagent` task is still outstanding.** A finished subagent's result is usually pushed to you automatically, but only if you make another tool call or model turn after it completes — if you stop first, that result is lost for good, not just delayed. Before ending your turn, call `check_subagent_status` on any task you haven't already seen a result or failure for. If it's still running, keep working on something else and check back rather than ending your turn — do not finish while a subagent you spawned is unaccounted for.
